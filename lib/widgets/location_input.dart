...
class LocationInput extends StatefulWidget {
  ...
  final void Function(ActivityLocation location) onSelectLocation;
...
class _LocationInputState extends State<LocationInput> {
  ActivityLocation? _pickedLocation;
  ...
  void _getCurrentLocation() async {
    ...
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$lat,$lng&key=AIzaSyCnmog83zJ9QlzfSyZhoFUy29lTxVbe67o');
    final response = await http.get(url);
    final resData = json.decode(response.body);
    final address = resData['results'][0]['formatted_address'];

    setState(() {
      _pickedLocation = ActivityLocation(
        latitude: lat,
        longitude: lng,
        address: address,
      );
